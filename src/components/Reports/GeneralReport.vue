    ...
    <v-card class="background pa-4 justify-center mb-3">
      <v-row v-if="transactions && transactions.length > 0" style="font-size: 0.8em" class="mt-n3">
        ...
        <v-col cols="4" class="text-right pt-1">
          ...
          <br/>
          <span>{{
              transactions[0].date.formatted | dateFilter
            }} | {{ transactions[transactions.length - 1].date.formatted | dateFilter }}</span>
        </v-col>
          ...
          <v-card class="pa-4 receipt-to-print">
            <v-row>
              ...
              <v-col cols="12" v-for="i in 2" :key="i" class="my-1 mx-0">
                ...
                <v-row
                    v-for="(productTransactions, index) in transactionByProduct.filter(p => i === 1 ? p.type === 'Product' : p.type === 'Bill')"
                    :key="index">
                  ...
                  <v-col cols="2" class="pa-0  text-center">{{
                      i === 1 ? productTransactions.cost :
                          -productTransactions.cost | moneyFilter
                    }}
                  </v-col>

                  <v-col v-if="i===1" cols="2" class="pa-0  text-center">{{
                      productTransactions.price
                          | moneyFilter
                    }}
                  </v-col>
                  ...
                  <v-col
                      v-if="(((productTransactions.price - productTransactions.cost)/productTransactions.cost)*100) >= 0"
              ...
              <v-col cols="12">
                <v-row class="background">
                  <v-col cols="2"></v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">DINHEIRO</v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">CRÉDITO</v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">DÉBITO</v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">PIX</v-col>
                </v-row>
                <v-row>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">TOTAL</v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">{{
                      payments.money|
                          moneyFilter
                    }}
                  </v-col>
                  ...
                  <v-col cols="2"></v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">TAXAS</v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">{{ 0| moneyFilter }}
                  </v-col>
                  <v-col cols="2" class="pa-0 text-center font-weight-bold">{{
      ...
      for (let transaction of this.transactions) {
        if (transaction.value > 0) {
          for (let payment in transaction.payment_methods) {
            ...
            } else if (transaction.payment_methods[payment] === 'Pix') {
              pix += transaction.payments[payment]
              taxaDebito += 0
            }
          }
        }

<template>
    <v-container>
        <v-layout row wrap>
            <v-flex xs12 class="px-0 my-3">
                <v-expansion-panels inset class="white">
                    <v-expansion-panel v-for="(user, i) in collaborators" :key="i">
                        <v-expansion-panel-header class="white">

                            <span style="font-size: small">{{ user.name.toUpperCase() }}</span>

                        </v-expansion-panel-header>
                        <v-expansion-panel-content class="background">
                            <v-layout row wrap>

                                <v-flex xs12 class="mt-2 justify-start text-start">
                                    <p>CPF: {{ user.cpf }}</p>
                                    <p class="mt-n3" v-if="user.telephones">Telefone: {{ user.telephones[0] }}</p>
                                </v-flex>
                                <!--                                <v-flex xs12>-->
                                <!--                                    <span v-if="user.bankData" class="mr-3">Banco: {{ user.bankData.bank.name }}</span>-->
                                <!--                                    <span v-if="user.bankData"-->
                                <!--                                          class="mr-3">Agência: {{ user.bankData.branchBank }}</span>-->
                                ...
                                <v-flex xs12 class="justify-space-between mb-2 mt-3">
                                    <v-btn small class="primary" @click="setSalary(user)" width="90%">Salario</v-btn>
                                    <br/>
                                    <v-btn  small class="primary mt-1 " @click="setAdvance(user)" width="90%">Adiantamento</v-btn>
                                    <br/>
                                    <v-btn small class="white--text red mt-1" :loading="loading"
                                           @click="deleteUser(user)" width="90%">Apagar
                                    </v-btn>
                                </v-flex>
                                    ...
                                    <v-layout row wrap>
                                        ...
                                        <v-flex xs12 v-if="user.advances">
                                            <v-layout row wrap>
                                                ...
                                                <v-flex xs3 class="my-2" v-for="advance in user.advances"
                                                        :key="advance.date">
                                                    <v-card class="pa-3">
                                                        <v-layout row wrap>
                                            ...
                                            <v-container fluid>
                                                <v-card>
                                                    <v-card-title class="headline grey lighten-2 mb-4" primary-title>
                                                        <v-btn style="display: none" text color="transparent"
                                                               class="transparent"/>
                                                        <v-spacer/>
                                                        ...
                                                        <v-layout row wrap>
                                                            ...
                                                            <v-flex xs6>
                                                                ...
                                                                <template v-else>
                                                                    ...
                                                                    <div v-for="(node,i) in selectedPermissions"
                                                                         :key="i">
                                                                        {{ node.name }}
                                                                    </div>
                                                                </template>
                                                    ...
                                                    <v-card-actions>
                                                        <v-flex xs12 class="text-center mb-2 mt-3">
                                                            <v-btn rounded text class="primary mx-1"
                                                                   @click="userGroups(user)">
                                                                Salvar
                                                            </v-btn>
                                                        </v-flex>
                                                    </v-card-actions>
                                        ...
                                        <v-layout row wrap>
                                            ...
                                            <v-flex xs12 class="text-center mb-2 mt-3">
                                                <v-btn rounded text class="primary mx-1" @click="dialog = true">Criar e
                                                    Editar Permissões
                                                </v-btn>
                                            </v-flex>

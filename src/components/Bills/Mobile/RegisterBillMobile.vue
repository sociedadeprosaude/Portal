<template>
  <v-card color="grey">
    <v-card-title class="primary text--white" primary-title>
      ...
      <v-flex xs5>
        <v-select label="Unidade" v-model="unit" :items="units" item-text="name" return-object></v-select>
      </v-flex>
      <v-flex xs2 class="transparent">
    ...
    <v-flex xs12 color="grey">
      <v-card color="grey">
        <v-layout row wrap class="align-center justify-center pa-3">
          ...
          <v-flex xs12>
            <v-select label="Método de pagamento" v-model="paymentMethod" :items="paymentMethods"></v-select>
          </v-flex>
          <v-flex xs6>

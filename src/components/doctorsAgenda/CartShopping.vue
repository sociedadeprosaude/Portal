    ...
    <v-container fluid class="ma-0 pa-0">
        <v-layout row nowrap style="width: 100%">
            <v-card v-for="(item, i) in Items" :key="i" class="my-2 grey_light" width="100%">
                ...
                <v-card-text v-if="categorySelect === 'package'" class="ma-0 pa-0 my-1">
                    ...
                    <v-divider/>
                    <v-layout row v-for="n in item.exams" :key="n.name" class="my-2 justify-center" style="width: 100%" v-if="item.exams">
                        <h5>{{n.name}}<v-spacer/></h5>
                    </v-layout>
                    <v-flex xs12 class="align-center justify-center text-center">
                        <span class="font-weight-bold" v-if="item.specialties.length !== 0">Consultas</span>
                    </v-flex>
                    <v-divider/>
                    <v-layout row v-for="n in item.specialties" :key="n.name" class="my-2 justify-center" style="width: 100%" v-if="item.specialties">
                        <h5 >{{n.name}}<v-spacer/></h5>
                    </v-layout>
                </v-card-text>
            </v-card>

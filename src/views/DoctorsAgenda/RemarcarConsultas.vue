                    ...
                    >
                        ...
                        <v-card class="mx-3" :color="consulta.color" dark>
                            ...
                            <v-card-text class="white text--primary">
                                ...
                                <v-btn dark round :color="consulta.color" class="mx-0" @click="scheduleAppointment = {crm:consulta.crm,medico:consulta.nome, data:consulta.data,hora:consulta.hora}">
                                    Agendar Consulta
                                </v-btn>
                            </v-card-text>
                        </v-card>
                            ...
                            <v-dialog v-model="dialog" width="500">
                                <v-card>
                                    <v-card-title class="headline grey lighten-2" primary-title>
                                        Confirmar Informações da Remarcação
                                    </v-card-title>
                                    <v-card-text>
...
<script>
    export default {
        ...
        computed: {
            ...
            loader(){
                return this.$store.getters.statusLoaderRemarcarConsulta
            },
            mensagem(){
        ...
        watch: {
            especialidade: async function (value) {
                ...
                await this.$store.dispatch('loadAppointment', {especialidade: value})
            },
            ...
            medicos: function (value) {
                ...
                this.datas = this.$store.getters.datas({especialidade: this.especialidade, medico: value, crm: crm})
                console.log(this.datas)
            },
            date: function (value) {
                this.timesOptions = this.$store.getters.horas({data: value})
            },

        },
        mounted() {
            //this.$store.dispatch('setLoader',{loader:false,view:"RemarcacaoConsulta"})
            this.query = this.$route.params.q
            if(!this.query){
               this.$router.push('/ConsultasCanceladas')
            }
        ...
        methods: {
            ...
            call2(){
                 this.$store.dispatch('scheduleAppointment',{remarcar:true,pacienteSelecionado: this.pacienteSelecionado,status:this.query.status,modalidade:this.query.modalidade, consulta: consulta, idConsulta:this.query.idConsultation,especialidade:this.especialidade,num_recibo: this.query.num_recibo})
            },
            formatDate (date) {
                if (!date) return null
            ...
            call(){
                ...
                var remarcar = this.query.modalidade == 'Consulta' ? true: false
                this.$store.dispatch('scheduleAppointment',{remarcar:remarcar,pacienteSelecionado: this.pacienteSelecionado,status:this.query.status,modalidade:this.query.modalidade, consulta: consulta, idConsulta:this.query.idConsultation,especialidade:this.especialidade,num_recibo: this.query.num_recibo})
                this.$store.dispatch('erase',{remarcar:remarcar,idConsultation:this.query.idConsultation,especialidade:this.especialidade})
               
                this.$router.push('/ConsultasCanceladas')
            },
            confirmar(){
                this.$store.dispatch('setLoader',{loader:true,view:"RemarcacaoConsulta"})

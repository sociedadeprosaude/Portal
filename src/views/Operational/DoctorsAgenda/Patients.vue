<template class="align-center">
    <v-container fab class="mt-0 pa-0">
        <v-card xs12 sm6 offset-sm3>
            <v-card-title>
                <v-icon left v-if="!cadastro"> search</v-icon>
                <h2 v-if="!cadastro">Buscar Paciente</h2>
                <v-spacer></v-spacer>
                <v-icon left v-if="cadastro"> group_add</v-icon>
                <h2 v-if="cadastro">Cadastrar Paciente</h2>
                <v-spacer></v-spacer>
                <v-tooltip bottom>
                    <template v-slot:activator=" { on }">
                        <v-btn round color="primary" small fab v-on="on" class="ma-3 mr-0"
                            ...
                            <v-icon v-if="cadastro">keyboard_arrow_up</v-icon>
                ...
                <v-tooltip bottom>
                    <template v-slot:activator=" { on } ">
                        <v-btn small fab v-show="!cadastro" v-on="on" round color="grey light"
                               class="text-xs-left mr-2 ml-0"
                               :disabled="loading"
                               :loading="loading"
            ...
            <v-flex xs12>
                <v-expand-transition>
                    <v-combobox class=" ml-3 pa-0" return-object full-width single-line hide-no-data hide-no-selected
                                clearable
                            ...
                            <v-flex xs12 sm9>
                                <h2 class="primary--text">{{buscarPaciente.nome}}
                                    <v-dialog v-model="EditarDados" max-width="600px">
                                        <template v-slot:activator="{on}">
                                            <v-btn flat icon color="blue lighten-2" v-on="on"
                                                   @click="editPaciente(buscarPaciente), EditarDados = true">
                                                <v-icon>edit</v-icon>
                                            </v-btn>
                                        </template>
                                        <v-card>
                                            <v-card-title><span
                                                    class="headline primary--text">{{editedPaciente.nome}}</span>
                                            </v-card-title>
                                            <v-card-text>
                                                    ...
                                                    <v-layout wrap>
                                                        <v-flex xs12 sm9>
                                                            <v-text-field label="Nome"
                                                                          v-model="editedPaciente.nome"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm3>
                                                            <v-text-field label="CPF" v-model="editedPaciente.cpf"
                                                                          :mask="mask.maskCpf"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm3>
                                                            <v-select label="Sexo" :items="['feminino' , 'masculino']"
                                                                      v-model="editedPaciente.sexo"></v-select>
                                                        </v-flex>
                                                        <v-spacer></v-spacer>
                                                        <v-flex xs12 sm4>
                                                            <v-text-field label="Data de Nascimento"
                                                                          v-model="editedPaciente.dataNascimento"></v-text-field>
                                                        </v-flex>
                                                        <v-spacer></v-spacer>
                                                        <v-flex xs12 sm3>
                                                            <v-text-field label="RG" v-model="editedPaciente.rg"
                                                                          :mask="mask.maskRG"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm8>
                                                            <v-text-field label="Email"
                                                                          v-model="editedPaciente.email"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm4>
                                                            <v-text-field label="Telefone"
                                                                          v-model="editedPaciente.telefone"
                                                                          :mask="mask.maskTelefone"
                                                        ...
                                                        <v-layout row fill-height v-show="expand">
                                                            <v-flex xs12 sm4>
                                                                <v-text-field label="Telefone"
                                                                              v-model="editedPaciente.telefone1"
                                                                              :mask="mask.maskTelefone"
                                                                              prepend-icon="phone"></v-text-field>
                                                            </v-flex>
                                                            <v-flex xs12 sm4>
                                                                <v-text-field label="Telefone"
                                                                              v-model="editedPaciente.telefone2"
                                                                              :mask="mask.maskTelefone"></v-text-field>
                                                            </v-flex>
                                                            <v-flex xs12 sm4>
                                                                <v-text-field label="Telefone"
                                                                              v-model="editedPaciente.telefone3"
                                                                              :mask="mask.maskTelefone"></v-text-field>
                                                            </v-flex>
                                                        </v-layout>
                                                        <v-flex xs12 sm3>
                                                            <v-text-field label="CEP" v-model="editedPaciente.cep"
                                                                          :mask="mask.maskCEP"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm3>
                                                            <v-combobox label="UF" v-model="editedPaciente.uf"
                                                                        :items="listasDados.estados"
                                                                        menu-props="auto"></v-combobox>
                                                        </v-flex>
                                                        <v-flex xs12 sm6>
                                                            <v-combobox label="Cidade" v-model="editedPaciente.cidade"
                                                                        :items="listasDados.cidades"
                                                                        menu-props="auto"></v-combobox>
                                                        </v-flex>
                                                        <v-flex xs12 sm10>
                                                            <v-text-field label="Logradouro"
                                                                          v-model="editedPaciente.logradouro"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm2>
                                                            <v-text-field label="Numero"
                                                                          v-model="editedPaciente.numero"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm6>
                                                            <v-switch v-model="existeDependente" :label="'Dependentes'"
                                                                      :append-icon="'group'"></v-switch>
                                                            <v-btn flat icon color="blue lighten-2"
                                                                   @click="dialogAddDependente = true">
                                                                <v-icon>add</v-icon>
                                                            </v-btn>
                                                            <v-dialog persistent v-model="dialogAddDependente">
                                                                <v-card color="white">
                                                                    <v-card-title>
                                                                        <h3 class="text-xs-center">Adicionar Dependente
                                                                            <v-icon right>group</v-icon>
                                                                        </h3>
                                                                    </v-card-title>
                                                                    <v-card-text>
                                                                        <v-form ref="dependentes"
                                                                                v-model="validDependente"
                                                                                lazy-validation>
                                                                                ...
                                                                                <v-layout wrap justify>
                                                                                    ...
                                                                                    <v-flex xs12 sm8>
                                                                                        <v-text-field required
                                                                                                      label="Responsável*"
                                                                                                      v-model="this.editedPaciente.nome"
                                                                                                      :counter="50"
                                                                                                      class="ml-3"
                                                                                                      maxlength="50"
                                                                                    ...
                                                                                    <v-flex xs12 sm3>
                                                                                        <v-select required
                                                                                                  ...
                                                                                                  v-model="dependente.grau"
                                                                                                  class="mr-3"
                                                                                                  :rules="[v => !!v || 'Selecione um valor']"
                                                                                                  :items="listasDados.grausDependentes"></v-select>
                                                                                    </v-flex>
                                                                                    ...
                                                                                    <v-spacer></v-spacer>
                                                                                    <v-flex xs12 sm3>
                                                                                        <v-select required label="Sexo*"
                                                                                                  v-model="dependente.sexo"
                                                                                                  :items="['feminino' , 'masculino']"
                                                                                                  class="mr-3"
                                                                                                  :rules="[v => !!v || 'Selecione um valor']"></v-select>
                                                                                    </v-flex>
                                                                    ...
                                                                    <v-card-actions>
                                                                        ...
                                                                        <v-btn color="primary" flat
                                                                               @click="validarDependente(),dialogAddDependente=false, dialogCadastro=true">
                                                                            Adicionar Dependente
                                                                        </v-btn>
                                                                        <v-btn color="primary" flat
                                                                               @click="dialogAddDependente=false">Fechar
                                                                        </v-btn>
                                                                    </v-card-actions>
                                                                </v-card>
                                                            </v-dialog>
                                                        ...
                                                        <v-layout row fill-height v-if="existeDependente">
                                                            <v-list two-line subheader>
                                                                <template v-for="(item) in editedPaciente.dependentes">
                                                                    <v-list-tile :key="item.nome" ripple
                                                                                 @click="editPaciente(item), EditarDados = true">
                                                                        <v-list-tile-avatar>
                                                                            <v-icon class="primary--text">person
                                                                            </v-icon>
                                                                        </v-list-tile-avatar>
                                                                        <v-list-tile-content>
                                                                            ...
                                                                            <v-list-tile-sub-title>{{item.grau}}
                    ...
                    <v-card class="mr-4 ml-4 mt-3" grid-list-x1 fluid>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            <v-layout wrap justify>
                                ...
                                <v-flex xs12 sm8>
                                    <v-text-field required label="Nome*" v-model="editedPaciente.nome" class="ml-3"
                                                  maxlength="50"
                                                  :counter="50"
                                                  :rules="rules.nomeRules && rules.CampoObrigatorio"></v-text-field>
                                </v-flex>
                                ...
                                <v-flex xs12 sm3>
                                    <v-menu ref="menu" v-model="menuData" :close-on-content-click="false"
                                        ...
                                        <template v-slot:activator="{on}">
                                            <v-text-field class="ml-3" v-model="editedPaciente.dataNascimento"
                                                          label="Data Nascimento" prepend-icon="event" readonly
                                                          v-on="on"></v-text-field>
                                        </template>
                                        <v-date-picker ref="picker" v-model="editedPaciente.dataNascimento"
                                                       :max="new Date().toISOString().substr(0,10)"
                                                       @input="menuData = false" locale="pt-br"></v-date-picker>
                                    </v-menu>
                                </v-flex>
                                <v-spacer></v-spacer>
                                <v-flex xs12 sm3>
                                    <v-text-field label="RG" v-model="editedPaciente.rg" :mask="mask.maskRG"
                                                  :counter="8"></v-text-field>
                                </v-flex>
                                <v-flex xs12 sm4>
                                    <v-text-field label="CPF" class="mr-3" v-model="editedPaciente.cpf"
                                                  :mask="mask.maskCpf" :counter="11"></v-text-field>
                                </v-flex>

                                <v-flex xs12 sm8>
                                    <v-text-field label="Email" class="ml-3" v-model="editedPaciente.email"
                                                  prepend-icon="email"></v-text-field>
                                </v-flex>
                                <v-flex xs12 sm4>
                                    <v-text-field required label="Telefone*" class="ml-2 mr-3"
                                                  v-model="editedPaciente.telefone" prepend-icon="phone"
                                                  :mask="mask.maskTelefone" :counter="11"
                                ...
                                <v-expand-transition>
                                    <v-layout row fill-height v-show="expand" class="mr-3 ml-3 text-xs-center">
                                        <v-flex xs12 sm3>
                                            <v-text-field label="Telefone" class="ma-3"
                                                          v-model="editedPaciente.telefone1" prepend-icon="phone"
                                                          :mask="mask.maskTelefone" :counter="11"></v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3>
                                            <v-text-field label="Telefone" class="ma-3"
                                                          v-model="editedPaciente.telefone2" prepend-icon="phone"
                                                          :mask="mask.maskTelefone" :counter="11"></v-text-field>
                                        </v-flex>
                                        <v-flex xs12 sm3>
                                            <v-text-field label="Telefone" class="ma-3"
                                                          v-model="editedPaciente.telefone3" prepend-icon="phone"
                                                          :mask="mask.maskTelefone" :counter="11"></v-text-field>
                                        </v-flex>
                                    </v-layout>
                                </v-expand-transition>
                                        ...
                                        <v-form>
                                            <v-container grid-list-x1 fluid>
                                                <v-layout wrap justify>
                                                    ...
                                                    <v-flex xs12 sm3>
                                                        <v-text-field label="CEP" class="ml-3"
                                                                      v-model="editedPaciente.cep" :mask="mask.maskCEP"
                                                                      :counter="8"></v-text-field>
                                                    </v-flex>
                                                    <v-spacer></v-spacer>
                                                    <v-flex xs12 sm3>
                                                        <v-select label="UF" hide-details single-line
                                                                  v-model="editedPaciente.uf"
                                                                  :items="listasDados.estados"
                                                                  menu-props="auto"></v-select>
                                                    </v-flex>
                                                    <v-flex xs12 sm4>
                                                        <v-select label="Cidade" class="mr-2" single-line
                                                                  v-model="editedPaciente.cidade"
                                                                  :items="listasDados.cidades" menu-props="auto"
                                ...
                                <v-spacer></v-spacer>
                                <v-btn round color="primary" dark class="ml-3 mt-4  text-xs-right" :disabled="!valid"
                                       @click="validate()">Adicionar Paciente
                                </v-btn>
                                <v-dialog v-model="dialogCadastro" hide-overlay max-width="500px">
                                    <v-card color="white">
                                        <v-card-title>
                                            <h3 class="text-xs-center">Paciente cadastrado com sucesso
                                        ...
                                        <v-card-actions>
                                            <v-spacer></v-spacer>
                                            <v-btn color="primary" flat @click="dialogCadastro=false, cancelar()"
                                ...
                                <v-dialog persistent v-model="dialogAddDependente">
                                    <v-card color="white">
                                            ...
                                            <v-form ref="dependentes" v-model="validDependente" lazy-validation>
                                                <v-container grid-list-x1 fluid>
                                                    <v-layout wrap justify>
                                                        ...
                                                        <v-flex xs12 sm8>
                                                            <v-text-field required label="Responsável*"
                                                                          v-model="this.editedPaciente.nome"
                                                                          :counter="50" class="ml-3" maxlength="50"
                                                                          :rules="rules.nomeRules && rules.CampoObrigatorio"></v-text-field>
                                                        </v-flex>
                                                        <v-spacer></v-spacer>
                                                        <v-flex xs12 sm3>
                                                            <v-select required label="Grau de Dependência*"
                                                                      v-model="dependente.grau" class="mr-3"
                                                                      :rules="[v => !!v || 'Selecione um valor']"
                                                                      :items="listasDados.grausDependentes"></v-select>
                                                        </v-flex>

                                                        <v-flex xs12 sm8>
                                                            <v-text-field required label="Nome*"
                                                                          v-model="dependente.nome" :counter="50"
                                                                          class="ml-3"
                                                                          :rules="rules.nomeRules && rules.CampoObrigatorio"
                                                                          maxlength="50"></v-text-field>
                                                        </v-flex>
                                                        <v-spacer></v-spacer>
                                                        <v-flex xs12 sm3>
                                                            <v-select required label="Sexo*" v-model="dependente.sexo"
                                                                      :items="['feminino' , 'masculino']"
                                                                      class="mr-3"
                                                                      :rules="[v => !!v || 'Selecione um valor']"></v-select>
                                                        </v-flex>
                                                        ...
                                                        <v-flex xs12 sm3>
                                                            <v-menu ref="menu" v-model="menu"
                                                                    ...
                                                                    lazy offset-y
                                                                    full-width transition="scale-transition"
                                                                    min-width="290px">
                                                                <template v-slot:activator="{ on }">
                                                                    <v-text-field required label="Data de Nascimento*"
                                                                                  v-model="dependente.dataNascimento"
                                                                                  class="ml-3" :mask="mask.data"
                                                                                  prepend-icon="date_range" v-on="on"
                                                                                  :rules="rules.CampoObrigatorio"></v-text-field>
                                                                </template>
                                                                <v-date-picker ref="picker" locale="pt-br"
                                                                               v-model="dependente.dataNascimento"
                                                                               :max="new Date().toISOString().substr(0, 10)"
                                                                               @input="menu = false"></v-date-picker>
                                                            </v-menu>
                                                        ...
                                                        <v-spacer></v-spacer>
                                                        <v-flex xs12 sm3>
                                                            <v-text-field label="RG" v-model="dependente.rg"
                                                                          :mask="mask.maskRG"
                                                                          :counter="8"></v-text-field>
                                                        </v-flex>
                                                        <v-flex xs12 sm4>
                                                            <v-text-field label="CPF" class="mr-3"
                                                                          v-model="dependente.cpf" :mask="mask.maskCpf"
                                                                          :counter="11"></v-text-field>
                                                        </v-flex>
                                        ...
                                        <v-card-actions>
                                            <v-spacer></v-spacer>
                                            <v-btn color="primary" flat
                                                   @click="validarDependente(),dialogAddDependente=false">Adicionar
                                                Dependente
                                            </v-btn>
                                        </v-card-actions>
                                    </v-card>
...
<script>
    export default {
        data: () => ({
            ...
            dialogSave: false,
            EditarDados: false,
            dialogAddDependente: false,
            dialogCadastro: false,
            dialogNovoProntuario: false,
            file: 'file',
            existeDependente: false,
            desserts: [],
            editedIndex: -1,
        ...
        methods: {
                ...
                if (this.buscarPaciente !== null) {
                    if (this.buscarPaciente === '' || this.buscarPaciente === undefined) {
                        ...
                        if (tamanho !== 0) {
                            ...
                            this.busca = false;
                            this.editedPaciente = Object.assign({}, this.defaultPaciente);
                            if (this.parameter === 'nome') {
                                this.editedPaciente.nome = this.buscarPaciente
                            }
                            ;
                            if (this.parameter === 'cpf') {
                                this.editedPaciente.cpf = this.buscarPaciente
                            }
                            ;
                            if (this.parameter === 'rg') {
                                this.editedPaciente.rg = this.buscarPaciente
                            }
                            ;
                            if (this.parameter === 'cartaoId') {
                                this.editedPaciente.cartaoId = this.buscarPaciente
                            }
                            ;
                            this.cadastro = true;
                        }
            ...
            addDependente() {
                const dependenteData = {
                    ...
                    responsavel: this.editedPaciente.nome,
                    dataNascimentoResponsavel: this.editedPaciente.dataNascimento,
                    email: this.editedPaciente.email,
                    telefone: this.editedPaciente.telefone,
                    telefone1: this.editedPaciente.telefone1,
                    telefone2: this.editedPaciente.telefone2,
